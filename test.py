...
class WebSocketServer:
    ...
    def start_server(self):
        start_server = websockets.serve(self.handle_command, "0.0.0.0", 8000)
    ...
    try:
        while True:
            ...
            for i, master in enumerate(sensor_manager.masters):
                ...
                print(f'Current [A]: {raw_current:.3f}')
                print(f'Power [W]: {power}')
                print(f'Energy [Wh]: {energy}')
                print(f'Energy [W/S]: {power_ps:.3f}')
                print(f'Energy [kW/H]: {power_kph:.3f}')
                print(f'Total [kWh]: {kiloenergy}')
