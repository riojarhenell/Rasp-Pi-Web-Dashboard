...
def main():
    ...
    try:
        while True:
            # Reading data from each sensor manually
            for i, (master, sensor) in enumerate(((master0, sensor0), (master1, sensor1), (master2, sensor2), (master3, sensor3))):
    ...
    for master in (master0, master1, master2, master3):
        try:
            master.close()
            if master.serial.is_open:
                master.serial.close()
        except:
            pass
